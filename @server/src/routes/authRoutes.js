...
function createAuthRoutes(pool) {
  ...
  router.post(
    "/send-verify-link",
    [body("username").trim().isLength({ min: 3 }), body("email").isEmail()],
    async (req, res) => {
      const errors = validationResult(req);
      if (!errors.isEmpty())
      ...
      try {
        ...
        res.json({
          message: "Verification email sent. Click the link to continue.",
        });
      } catch (e) {
  ...
  router.get("/verify-email/:token", async (req, res) => {
    ...
    try {
      ...
      if (user.emailVerified) {
        await markTokenUsed(pool, tokenDoc._id);
        return res.json({
          message: "Email already verified. You can continue.",
        });
      }
